      subroutine hybrd1(fcn,n,lr,x,fvec,jmat,tol,info,wa,lwa)
      integer n,info,lwa,lr
      double precision tol
      double precision x(n),fvec(n),wa(lwa),jmat(n,n)
      external fcn
c     **********
c
c     subroutine hybrd1
...
c
c         info = 4   iteration is not making good progress in the last 5 iteration.
c         info = 5   iteration is not making good progress in the last 10 iteration.
c
...
c
c     **********
      integer index,j,maxfev,ml,mode,mu,nfev,nprint,i,k
      double precision epsfcn,factor,one,xtol,zero,q(n,n),r(lr)
...
c
   ...
   10    continue
     ...
     *           factor,nprint,info,nfev,q,n,r,lr,
     *           wa(n+1),wa(2*n+1),wa(3*n+1),wa(4*n+1),wa(5*n+1))
c      if (info .eq. 5) info = 4
   20 continue
      do 60 i=1,n
         do 50 j=1,n
            jmat(i,j)=0
            do 40 k=1,j
               jmat(i,j)=jmat(i,j)+q(i,k)*r(n*(k-1)-(k-2)*(k-1)/2+j-k+1)
 40         continue
 50      continue
 60   continue
      return
