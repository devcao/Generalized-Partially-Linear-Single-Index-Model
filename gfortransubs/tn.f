...
C PRINT THE RESULTS
C
C      IF (IERROR .NE. 0) WRITE(*,800) IERROR
C      WRITE(*,810) F
...
C THE TRUNCATED-NEWTON METHOD IS PRECONDITIONED BY A LIMITED-MEMORY
C QUASI-NEWTON METHOD (THIS PRECONDITIONING STRATE,GY IS DEVELOPED
C IN THIS ROUTINE) WITH A FURTHER DIAGONAL SCALING (SEE ROUTINE NDIA3).
...
10    CONTINUE
      ...
      GTG = DDOT(N,G,1,G,1)
      IF (MSGLVL .GE. 1)
     *    CALL MONIT(N,X,FNEW,G,NITER,NFTOTL,NFEVAL,IPIVOT)
C
...
150   F = OLDF
     ...
     *     F,G,NITER,NFTOTL,NFEVAL,IPIVOT)
C
C SET IFAIL
C
160   IFAIL = NWHY
...
C
      ...
      DO 10 I = 1,N
         ...
         GTG = GTG + G(I)*G(I)
10    CONTINUE
C      WRITE(*,800) NITER,NFTOTL,NFEVAL,F,GTG
      RETURN
